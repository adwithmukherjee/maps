...
public class SQLQueries {
  ...
  public static String ways(Double lat1, Double long1, Double lat2, Double long2){
    return "" +
        ...
        "LEFT JOIN node as m\n" +
        "ON w.end = m.id\n" +
        "WHERE ((n.latitude <= "+lat1+" AND n.latitude >="+lat2+") AND (n.longitude >= "+long1+" AND n.longitude <= "+long2+"))\n" +
        "OR ((m.latitude <= "+lat1+" AND m.latitude >="+lat2+") AND (m.longitude >= "+long1+" AND m.longitude <="+long2+"))";
  }
  ...
  public static String routeEdges(Double maxLat, Double minLong, Double minLat, Double maxLong){
    return "" +
        ...
        "ON w.end = m.id\n" +
        "WHERE ((w.type != \"\")" +
        "AND (((n.latitude <= "+maxLat+" AND n.latitude >="+minLat+") AND (n.longitude >= "+minLong+" AND n.longitude <= "+maxLong+"))\n" +
        "OR ((m.latitude <= "+maxLat+" AND m.latitude >="+minLat+") AND (m.longitude >= "+minLong+" AND m.longitude <="+maxLong+"))))";
  }
